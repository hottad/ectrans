...
!     LDUSEFJ    - Use Fejer grid (dual of Clenshaw-Curtis) instead of Gauss or CC
...
IF(PRESENT(LDUSECC).AND.PRESENT(LDUSEHLPX)) THEN
  ...
  ENDIF
  IF (PRESENT(LDUSEFJ).AND.LDUSEFJ) THEN
    IF(LDUSECC.OR.LDUSEHLPX) THEN
      CALL  ABORT_TRANS('SETUP_TRANS: AT MOST ONE OF LDUSECC, LDUSEHLPX AND LDUSEFJ CAN BE .TRUE. ')
    ENDIF
  ENDIF
ENDIF
IF(PRESENT(LDUSEHLPX)) THEN
